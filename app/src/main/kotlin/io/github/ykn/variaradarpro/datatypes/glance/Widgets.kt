...
class ApproachSpeedGlanceDataType(
    ...
    override fun Content(input: RenderInput, config: ViewConfig) {
        ...
        val relative = VariaRadarExtension.toUserSpeedUnits(input.closingSpeedMps, input.useImperial)
        val absolute = if (relative > 0) {
            relative + VariaRadarExtension.toUserSpeedUnits(input.riderSpeedMps, input.useImperial)
        } else {
            0
        }
